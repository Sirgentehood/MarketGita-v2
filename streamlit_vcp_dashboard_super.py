...
# Private super-dashboard contrast/action/F&O override.
st.markdown("""
<style>
.action-pill {display:inline-block; font-size:0.74rem; font-weight:900; padding:0.18rem 0.52rem; border-radius:999px; white-space:nowrap; margin-top:0.18rem; border:1px solid rgba(255,255,255,0.24);}
.action-buy {background: rgba(30,201,119,0.16); color: #31e894; border-color: rgba(30,201,119,0.38);}
.action-hold {background: rgba(110,168,254,0.16); color: #93c5fd; border-color: rgba(110,168,254,0.38);}
.action-sell {background: rgba(255,107,107,0.16); color: #ff9b9b; border-color: rgba(255,107,107,0.38);}
.action-neutral {background: rgba(240,180,41,0.16); color: #ffd166; border-color: rgba(240,180,41,0.38);}
.fo-pill {display:inline-block; font-size:0.72rem; font-weight:900; padding:0.16rem 0.48rem; border-radius:999px; white-space:nowrap; margin-top:0.18rem; border:1px solid rgba(255,255,255,0.20); background:rgba(255,255,255,0.08); color:#eef3ff;}
.fo-yes {background:rgba(183,156,255,0.18); color:#d8c9ff; border-color:rgba(183,156,255,0.42);}
.action-reason-line {font-size:0.82rem; color:rgba(255,255,255,0.74); line-height:1.28; margin-top:0.18rem;}
/* Fix fields/labels that disappear until hover in Streamlit dark mode. */
.stSelectbox label, .stMultiSelect label, .stSlider label, .stCheckbox label, .stToggle label, .stTextInput label, .stTextArea label, .stRadio label {color:#f8fafc !important;}
[data-testid="stWidgetLabel"], [data-testid="stMarkdownContainer"] p {color:inherit;}
...
[data-testid="stDataFrame"] {background:rgba(255,255,255,0.04) !important; border-radius:14px !important;}
</style>
""", unsafe_allow_html=True)
...
def prepare_internal_setup_df(internal_df: pd.DataFrame) -> pd.DataFrame:
    if internal_df is None or internal_df.empty:
        return pd.DataFrame()
    out = normalize_columns(internal_df.copy())
    rename_map = {
...
def merge_internal_setup(combined_df: pd.DataFrame, internal_df: pd.DataFrame) -> pd.DataFrame:
    if combined_df is None or combined_df.empty:
        return combined_df
    internal_df = prepare_internal_setup_df(internal_df)
    if internal_df.empty or "ticker" not in internal_df.columns:
        return combined_df
    keep = ["ticker"] + [c for c in [
    ...
    extra = internal_df[keep].drop_duplicates("ticker")
    out = combined_df.merge(extra, on="ticker", how="left", suffixes=("", "_internal"))
    for col in ["internal_leader_score", "internal_trend_score", "internal_setup_score", "internal_risk_score", "internal_quality_score", "risk_pct"]:
        if col in out.columns:
            out[col] = pd.to_numeric(out[col], errors="coerce")
    for col in ["internal_is_true_leader", "internal_is_proper_setup", "internal_is_low_risk", "internal_is_buyable_setup"]:
        if col in out.columns:
            out[col] = out[col].apply(boolish)
    return out


def simple_private_action(row: pd.Series) -> tuple[str, str, str]:
    """Private-facing action label for dashboard cards: Buy / Hold / Sell / Neutral."""
    raw_action = str(row.get("action", "") or "").strip()
    if raw_action in {"Buy", "Tactical Buy"}:
        return "Buy", "Decision engine marks this as a long-side action.", "buy"
    if raw_action in {"Short", "Tactical Short"}:
        return "Sell", "Decision engine marks this as a short/exit-side action.", "sell"
    if raw_action == "Watch for Long":
        return "Hold", "Structure is constructive but not a clean Buy signal yet.", "hold"
    if raw_action == "Watch for Short":
        return "Sell", "Structure is deteriorating enough to stay defensive.", "sell"
    if raw_action == "No Trade":
        return "Neutral", "No clear private directional edge from current model evidence.", "neutral"

    stage = str(row.get("stage", "")).strip()
    stage_variant = str(row.get("stage_variant", row.get("stage_classification", "")) or "").lower()
    quality = pd.to_numeric(row.get("internal_quality_score", row.get("final_combined_score", row.get("combined_score"))), errors="coerce")
    rs3 = pd.to_numeric(row.get("rs_3m_pct"), errors="coerce")
    risk = pd.to_numeric(row.get("risk_pct"), errors="coerce")
    buyable = boolish(row.get("internal_is_buyable_setup", False))
    leader = boolish(row.get("internal_is_true_leader", False))
    proper = boolish(row.get("internal_is_proper_setup", False))

    if buyable and stage in {"Stage 1", "Stage 2"}:
        return "Buy", "Internal setup gate passed: leadership, setup quality and risk are acceptable.", "buy"
    if stage == "Stage 2" and leader and proper and pd.notna(quality) and quality >= 72 and (pd.isna(risk) or risk <= 8):
        return "Buy", "Stage 2 leader with proper setup and controlled risk.", "buy"
    if stage == "Stage 2":
        return "Hold", "Advancing structure is intact, but the private low-risk setup gate has not fully passed.", "hold"
    if stage in {"Stage 2 Failed", "Failed Stage 2", "Stage 4"}:
        return "Sell", "Structure is damaged or declining in the private model.", "sell"
    if stage == "Stage 3":
        if "late" in stage_variant or (pd.notna(rs3) and rs3 < -5) or (pd.notna(quality) and quality < 45):
            return "Sell", "Stage 3 with late/damaged evidence or weak relative strength.", "sell"
        return "Neutral", "Transition structure; no clean Buy/Sell edge yet.", "neutral"
    if stage == "Stage 1":
        return "Neutral", "Base-building phase; wait for stronger confirmation.", "neutral"
    return "Neutral", "No clear private directional edge from current model evidence.", "neutral"


def action_counts_from_df(df: pd.DataFrame) -> dict:
    if df is None or df.empty:
        return {"Buy": 0, "Hold": 0, "Sell": 0, "Neutral": 0}
    vals = df.apply(lambda r: simple_private_action(r)[0], axis=1)
    return {k: int(vals.eq(k).sum()) for k in ["Buy", "Hold", "Sell", "Neutral"]}


def fo_label_for_row(row: pd.Series) -> str:
    return "F&O" if boolish(row.get("is_fo_stock", False)) else "Non F&O"
...
def render_trade_card(row: pd.Series):
    ...
    simple_action, action_reason, action_style = simple_private_action(row)
    fo_text = fo_label_for_row(row)
    industry_name = str(row.get("Industry", "")).strip()
...
<div class='stock-card {_stage_card_class(str(row.get("stage", "")))}'>
  <div style='display:flex; justify-content:space-between; align-items:flex-start; gap:0.6rem;'>
    <div style='min-width:0;'>
      ...
      <div class='meta-line'><span class='action-pill action-{action_style}'>{simple_action}</span> • Engine: {action} • {side} • Score {score}/100</div>
      <div class='stock-subtitle'>{str(row.get("rationale", ""))}</div>
      <div class='action-reason-line'>{action_reason}</div>
      <div class='small-note' style='margin-top:0.2rem;'>{industry_icon(industry_name)} {industry_name or 'Unknown industry'} • {industry_view} • <span class='fo-pill {'fo-yes' if fo_text == 'F&O' else ''}'>{fo_text}</span></div>
      {mini_html}
...
def card(row: pd.Series, pct=None, use_stage_color=False, show_change_text: str = "", stock_rank: str = "n/a", action_label: str = ""):
    ...
    action_label = action_label or simple_action
    action_html = f"<div class='action-pill action-{action_style}' style='margin-top:0.2rem;'>{action_label}</div>"
    fo_html = f"<div class='fo-pill {'fo-yes' if fo_text == 'F&O' else ''}'>{fo_text}</div>"
    ...
    html = (
        ...
        f"<div class='small-note' style='margin-top:0.2rem;'>{industry_with_icon} • {fo_text}</div>"
        f"<div class='action-reason-line'>{action_reason}</div>"
        f"{signals_html}{mini_html}"
        f"</div>"
        f"<div style='display:flex; flex-direction:column; align-items:flex-end; gap:0.05rem;'>"
        f"{status_html}{action_html}{fo_html}{rank_html}{rank_change_html}{change_html}"
        f"</div>"
        f"</div>"
        f"{extra_change}"
        f"</div>"
...
internal_setup = ensure_label(safe_read(f"{outdir}/internal_setup_ranked.csv"))
combined = merge_internal_setup(combined, internal_setup)
...
if not DECISION_DF.empty:
    _action_parts = DECISION_DF.apply(simple_private_action, axis=1)
    DECISION_DF["private_action"] = _action_parts.apply(lambda x: x[0])
    DECISION_DF["private_action_reason"] = _action_parts.apply(lambda x: x[1])
    DECISION_DF["private_action_style"] = _action_parts.apply(lambda x: x[2])
...
with tabs[0]:
    ...
    simple_counts = action_counts_from_df(DECISION_DF if not DECISION_DF.empty else combined)
    ...
    with c3:
        render_summary_card("Buy", str(simple_counts["Buy"]), "Private directional action")
    with c4:
        render_summary_card("Hold", str(simple_counts["Hold"]), "Private directional action")
    with c5:
        render_summary_card("Sell", str(simple_counts["Sell"]), "Private directional action")
    with c6:
        render_summary_card("Neutral", str(simple_counts["Neutral"]), "Private directional action")
...
with tabs[1]:
    ...
    if DECISION_DF.empty:
        ...
        show_cols = [c for c in [
            "Company Name", "ticker", "Industry", "stage", "stage_classification", "label", "private_action", "action", "trade_side",
            "long_score", "short_score", "action_confidence", "industry_score", "industry_view",
            "internal_quality_score", "internal_failure_reasons", "current_rank", "rank_change", "is_fo_stock", "dry_up_status", "mini_signals", "rationale"
